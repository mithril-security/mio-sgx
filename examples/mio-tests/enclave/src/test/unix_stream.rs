#![cfg(all(any(unix, target_env = "sgx"), feature = "os-poll", feature = "net"))]
...
//#[test]
pub fn unix_stream_send_and_sync() {
    assert_send::<UnixStream>();
    assert_sync::<UnixStream>();
}

//#[test]
pub fn unix_stream_smoke() {
...
//#[test]
pub fn unix_stream_shutdown_read() {
    let (mut poll, mut events) = init_with_poll();
...
//#[test]
pub fn unix_stream_shutdown_write() {
    let (mut poll, mut events) = init_with_poll();
...
//#[test]
pub fn unix_stream_shutdown_both() {
    let (mut poll, mut events) = init_with_poll();
...
//#[test]
pub fn unix_stream_shutdown_listener_write() {
    let (mut poll, mut events) = init_with_poll();
    let barrier = Arc::new(Barrier::new(2));
