...
use crate::test::util::{assert_send, assert_sync, expect_no_events, init};
...
//#[test]
pub fn waker_multiple_wakeups_same_thread() {
    init();
...
//#[test]
pub fn waker_wakeup_different_thread() {
    init();
    ...
    let token = Token(10);
    let waker = Waker::new(poll.registry(), token).expect("unable to create waker");
    // println!("Creating waker.");
    let waker = Arc::new(waker); // Error is here!
...
//#[test]
pub fn waker_multiple_wakeups_different_thread() {
    init();
