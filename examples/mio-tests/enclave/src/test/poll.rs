...
//#[test]
pub fn zero_duration_polls_events() {
    init();
...
//#[test]
pub fn registry_operations_are_thread_safe() {
...
//#[test]
pub fn reregister_interest_token_usage() {
    let (mut poll, mut events) = init_with_poll();
...
//#[test]
pub fn poll_ok_after_cancelling_pending_ops() {
    let (mut poll, mut events) = init_with_poll();
...
//#[test]
#[cfg(debug_assertions)] // Check is only present when debug assertions are enabled.
pub fn reregister_without_register() {
...
//#[test]
#[cfg(debug_assertions)] // Check is only present when debug assertions are enabled.
pub fn deregister_without_register() {
    let poll = Poll::new().expect("unable to create Poll instance");
...
//#[test]
pub fn poll_erroneous_registration() {
    init();
    let poll = Poll::new().unwrap();
    let registry = poll.registry();
