...
//#[test]
pub fn get_nodelay_without_previous_set() {
    let (mut poll, mut events) = init_with_poll();
...
//#[test]
pub fn no_events_after_deregister() {
    let (mut poll, mut events) = init_with_poll();
...
pub fn tcp_shutdown_client_read_close_event() {
    let (mut poll, mut events) = init_with_poll();
    let barrier = Arc::new(Barrier::new(2));
...
//#[test]
pub fn tcp_shutdown_server_write_close_event() {
    let (mut poll, mut events) = init_with_poll();
    let barrier = Arc::new(Barrier::new(2));
...
pub fn tcp_shutdown_client_both_close_event() {
    let (mut poll, mut events) = init_with_poll();
    let barrier = Arc::new(Barrier::new(2));
...
//#[test]
pub fn hup_event_on_disconnect() {
